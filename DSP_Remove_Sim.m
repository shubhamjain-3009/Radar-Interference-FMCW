...
title('Chirp with Interference: Time Domain')
xlabel('Sample index')
axis([1 512 -inf inf])
hold on
xline(start)
...
title('Chirp without Interference: Time Domain')
xlabel('Sample index')
axis([1 512 -inf inf])
...
wi = (kaiser(N_sample,19))';
wi = wi/sum(wi);
RangeFFT_mat = fft(adcn_mat_noint_ds.*wi,N_fft,2);
RangeFFTn = RangeFFT_mat(1,:);
plot(db(RangeFFTn(1,:)),'b','LineWidth',1);
avg_nn = mean(db(RangeFFTn(1,:)));

N_fft = N_sample;
wi = (kaiser(N_sub+1,19))';
wi = wi/sum(wi);
adcn_mat_ds_sub = adcn_mat_ds(1,start:start+N_sub);
RangeFFT_mat = fft(adcn_mat_ds_sub.*wi,N_fft,2);
RangeFFTn = RangeFFT_mat(1,:);
plot(db(RangeFFTn(1,:)),'r:','LineWidth',1);
avg_nc = mean(db(RangeFFTn(1,:)));

wi = (kaiser(N_sample,19))';
wi = wi/sum(wi);
RangeFFT_mat = fft(adcn_mat_ds.*wi,N_fft,2);
RangeFFTn = RangeFFT_mat(1,:);
plot(db(RangeFFTn(1,:)),'k');
avg_wn = mean(db(RangeFFTn(1,:)));

legend("Without Int", "With Int and Time Domain Omission", "With Int")
title('Range FFT')
xlabel('FFT Bin')
ylabel('Magnitude (dB)')
[avg_nn,avg_wn,avg_nc]
